...
public:
	...
	inline Sphere(float radius, int n, glm::vec3 coords) : Object(coords), radius(radius), n(n), rotX(0), rotY(0), rotZ(0), local(System(1, coords))
	{}
	inline Sphere(float radius, int n, glm::vec3 coords, float rx, float ry, float rz) : Object(coords), radius(radius), n(n), rotX(rx), rotY(ry), rotZ(rz), local(System(1, coords))
	{}
	...
	void global_rotate_x() {
		float y = (coords.y * cos(0.05f)) - (coords.z * sin(0.05f));
		float z = (coords.y * sin(0.05f)) + (coords.z * cos(0.05f));
		coords.y = y;
		coords.z = z;
		this->model = glm::translate(glm::mat4(1.0f), coords);
		local.model = glm::translate(glm::mat4(1.0f), coords);
	}

	void global_rotate_y() {
		float x = (coords.x * cos(0.05f)) + (coords.z * sin(0.05f));
		float z = (-coords.x * sin(0.05f)) + (coords.z * cos(0.05f));
		coords.x = x;
		coords.z = z;
		this->model = glm::translate(glm::mat4(1.0f), coords);
		local.model = glm::translate(glm::mat4(1.0f), coords);
	...
	void global_rotate_z() {
		float x = (coords.x * cos(0.05f)) - (coords.y * sin(0.05f));
		float y = (coords.x * sin(0.05f)) + (coords.y * cos(0.05f));
		coords.x = x;
		coords.y = y;
		this->model = glm::translate(glm::mat4(1.0f), coords);
		local.model = glm::translate(glm::mat4(1.0f), coords);
